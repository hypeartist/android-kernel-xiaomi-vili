			   ...
			   int *flags)
{
	...
	*hdr = nl80211hdr_put_qcacld(msg, portid, 0, *flags, WLAN_NL_MSG_OEM);
	if (*hdr == NULL) {
		QDF_TRACE(QDF_MODULE_ID_HDD, QDF_TRACE_LEVEL_ERROR,
...
void *nl80211hdr_put_qcacld(struct sk_buff *skb, uint32_t portid,
					...
					uint8_t *buf, int len)
{
	...
	hdr = nl80211hdr_put_qcacld(msg, portid, seq, flags, cmd);
	if (!hdr) {
		QDF_TRACE(QDF_MODULE_ID_HDD, QDF_TRACE_LEVEL_ERROR,
						"nl80211 hdr put failed");
		return -EPERM;
